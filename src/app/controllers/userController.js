...
exports.signUp = async function (req, res) {
    ...
    if (!email) 
        return res.json({isSuccess: false, code: 2000, message: "이메일을 입력해주세요"});
    
    if (email.length > 30) 
        return res.json(
            {isSuccess: false, code: 2001, message: "이메일은 30자리 미만으로 입력해주세요"}
        );
    
    if (!regexEmail.test(email)) 
        return res.json(
            {isSuccess: false, code: 2002, message: "이메일을 형식을 정확하게 입력해주세요"}
        );
    ...
    if (password.length < 8 || password.length > 20) 
        return res.json(
            {isSuccess: false, code: 2004, message: "비밀번호는 8자 이상 20자 이하로 입력해주세요"}
        );
    
    if (!passwordCheck) 
        return res.json({isSuccess: false, code: 2005, message: "비밀번호를 한번 더 입력해주세요"});
    
    if (passwordCheck !== password) 
        return res.json({isSuccess: false, code: 2006, message: "비밀번호가 맞지 않습니다"});
    
    if (!nickname) 
        return res.json({isSuccess: false, code: 2007, message: "닉네임을 입력 해주세요"});
    
    var englishCheck = /[a-zA-Z]/gi;

    if (!/^([가-힣]).{1,8}$/.test(nickname) || englishCheck.test(nickname)) 
        return res.json(
            {isSuccess: false, code: 2008, message: "닉네임은 한글만 입력가능하고 2자 이상 8자 이하 이어야 합니다"}
        );
    ...
    if (nickname.search(/\s/) != -1 || specialPattern.test(nickname) == true || checkSpc.test(nickname) == true) 
        return res.json(
            {isSuccess: false, code: 2009, message: "닉네임에는 공백 또는 특수문자를 입력할 수 없습니다."}
        );
    ...
    try {
        ...
        if (emailRows.length > 0) {
            ...
            return res.json({isSuccess: false, code: 3000, message: "사용 불가능한 이메일입니다"});
        }

        // 닉네임 중복 확인
        const nicknameRows = await userDao.userNicknameCheck(nickname);
        if (nicknameRows.length > 0) {
            return res.json({isSuccess: false, code: 3001, message: "이미 사용중인 닉네임입니다"});
        }
        ...
        return res.json({isSuccess: true, code: 1000, message: "회원가입 성공"});
    } catch (err) {
...
exports.signIn = async function (req, res) {
    ...
    try {
        ...
        if (userInfoRows.length < 1) {
            ...
            return res.json({isSuccess: false, code: 3000, message: "존재하지 않는 아이디 입니다."});
        }
        ...
        if (userInfoRows[0].password !== hashedPassword) {
            ...
            return res.json({isSuccess: false, code: 3001, message: "비밀번호를 확인해주세요."});
        }
        if (userInfoRows[0].status === 0) {

            return res.json(
                {isSuccess: false, code: 3002, message: "비활성화 된 계정입니다. 고객센터에 문의해주세요."}
            );
        }
        //토큰 생성
...
exports.check = async function (req, res) {
    res.json(
        {isSuccess: true, code: 1000, message: "검증 성공", info: req.verifiedToken}
    )
};
...
exports.myPage = async function (req, res) {
    ...
    try {
        ...
        //마이페이스 내가 북마크 한 글
        const userBookmarkWritingRows = await userDao.getUserBookmarkWritingRows(
            userIdx
        );

        //마이페이스 내가 북마크 한 서점
        const userBookmarkBookstoreRows = await userDao.getUserBookmarkBookstore(
            userIdx
        );

        if (userInfoRows.length >= 0 && userWritingRows.length >= 0 && userBookmarkWritingRows.length >= 0 && userBookmarkBookstoreRows.length >= 0) 
            return res.json({
                isSuccess: true,
                code: 1000,
                ...
                result: {
                    info: userInfoRows,
                    writing: userWritingRows,
                    writingBookmark: userBookmarkWritingRows,
                    bookstoreBookmark: userBookmarkBookstoreRows
                }
            });
...
exports.changePassword = async function (req, res) {
    ...
    if ((!currentPassword) || (!newPassword) || (!passwordCheck)) 
        return res.json(
            {isSuccess: false, code: 2000, message: "currentPassword, newPassword, passwordCheck 를 모두 입력해 주세요."}
        );
    ...
    try {
        ...
        if (userInfoRows[0].password !== hashedPassword) {
            ...
            return res.json({isSuccess: false, code: 3001, message: "현재 비밀번호를 확인해주세요."});
        }

        if (newPassword.length < 8 || newPassword.length > 20) 
            return res.json(
                {isSuccess: false, code: 2002, message: "비밀번호는 8자 이상 20자 이하로 입력해주세요"}
            );
        
        if (newPassword !== passwordCheck) 
            return res.json({isSuccess: false, code: 2001, message: "두 비밀번호가 맞지 않습니다."});
        ...
        if (hashedPassword == newHashedPassword) 
            return res.json(
                {isSuccess: false, code: 2003, message: "변경전 비밀번호와 변경후 비밀번호가 같습니다."}
            );
        
        const newInfoParams = [newHashedPassword, userIdx];
        const newUserInfoRows = await userDao.updateUserPasswordInfo(newInfoParams)

        return res.json({isSuccess: true, code: 1000, message: "비밀번호 변경 성공"});
...
exports.changeNickname = async function (req, res) {
    ...
    if (!nickname) 
        return res.json({isSuccess: false, code: 2000, message: "닉네임을 입력 해주세요"});
    
    var englishCheck = /[a-zA-Z]/gi;

    if (!/^([가-힣]).{1,8}$/.test(nickname) || englishCheck.test(nickname)) 
        return res.json(
            {isSuccess: false, code: 2001, message: "닉네임은 한글만 입력가능하고 2자 이상 8자 이하 이어야 합니다"}
        );
    ...
    if (nickname.search(/\s/) != -1 || specialPattern.test(nickname) == true || checkSpc.test(nickname) == true) 
        return res.json(
            {isSuccess: false, code: 2002, message: "닉네임에는 공백 또는 특수문자를 입력할 수 없습니다."}
        );
    ...
    try {
        ...
        if (currentNickname == nickname) {
            return res.json(
                {isSuccess: false, code: 2003, message: "변경전 닉네임과 변경후 닉네임이 같습니다."}
            );
        }
...
exports.patchImage = async function (req, res) {
    ...
    if (!image) 
        return res.json(
            {isSuccess: false, code: 2000, message: "변경할 이미지의 URL을 입력해 주세요."}
        );
    ...
    try {
        ...
        return res.json({isSuccess: true, code: 1000, message: "프로필 이미지 변경 완료"});
    } catch (err) {
...
exports.deleteImage = async function (req, res) {
    ...
    try {
        ...
        if (userInfoRows[0].userImgUrl == -1 || userInfoRows[0].userImgUrl == null) 
            return res.json({isSuccess: false, code: 3000, message: "삭제할 프로필 이미지가 없습니다."});
        
        //이미지 url를 -1로 변경
        const updateImageParams = [-1, userIdx];
        const updateImageRows = await userDao.updateImage(updateImageParams);

        return res.json({isSuccess: true, code: 1000, message: "프로필 이미지 삭제 완료"});
    } catch (err) {
...
exports.patchUserStatus = async function (req, res) {
    ...
    try {
        ...
        return res.json({isSuccess: true, code: 1000, message: "회원 탈퇴 완료"});
    } catch (err) {
...
//내가쓴글조회 특정 bookIdx에서 내가 쓴 글만 조회 하는 기능
exports.getUserWriting = async function (req, res) {
    ...
    if (!/^([0-9]).{0,10}$/.test(bookIdx)) 
        return res.json(
            {isSuccess: false, code: 2001, message: "bookIdx는 숫자로 입력해야 합니다."}
        );
    
    if ((!page) || (!limit)) 
        return res.json(
            {isSuccess: false, code: 2002, message: "page와 limit을 입력해 주세요."}
        );
    ...
    try {
        ...
        if (checkBookIdxRows.length == 0) 
            return res.json(
                {isSuccess: false, code: 2000, message: "해당하는 인덱스의 책이 존재 하지 않습니다."}
            );
        
        const getUserWritingParams = [userIdx, bookIdx, userIdx, Number(page), Number(limit)];

        //유저가 쓴글을 가져옴
        const getUserWritingInfoRows = await userDao.getUserWritingInfo(
        ...
        if (getUserWritingInfoRows.length > 0) 
            return res.json({
                ...
                code: 1000,
                result: {
                    book: getBookNameRows,
                    writing: getUserWritingInfoRows
                },
                message: "내가 쓴글 조회 완료"
            });
        
        return res.json(
            {isSuccess: false, code: 4000, message: "해당 인덱스에는 내가 쓴글이 없습니다."}
        );
    } catch (err) {
...
//내가 북마크 한 글 조회 특정 bookIdx에서 내가 북마크하는 글만 조회 하는 기능
exports.getUserBookmarkWriting = async function (req, res) {
    ...
    try {
        ...
        //유저가 북마크 한 글을 가져옴
        const getBookmarkWritingInfoRows = await userDao.getBookmarkWritingInfo(
            getUserWritingParams
        );
        //책 이름 정보 가져옴
        const getBookNameRows = await userDao.getBookName(bookIdx);
        ...
        if (getBookmarkWritingInfoRows.length > 0) 
            return res.json({
                ...
                result: {
                    myUserIdx: userIdx,
                    book: getBookNameRows,
                    writing: getBookmarkWritingInfoRows
                },
                message: "내가 북마크 한 글 조회 완료"
            });
        
        return res.json(
            {isSuccess: false, code: 4000, message: "해당 인덱스에는 내가 북마크 한 글이 없습니다."}
        );
    } catch (err) {
...
exports.findPassword = async function (req, res) {
    ...
    if (!email) 
        return res.json({isSuccess: false, code: 2000, message: "이메일을 입력해 주세요."});
    
    if (!nickname) 
        return res.json({isSuccess: false, code: 2001, message: "닉네임을 입력 해주세요"});
    
    var englishCheck = /[a-zA-Z]/gi;

    if (!/^([가-힣]).{1,8}$/.test(nickname) || englishCheck.test(nickname)) 
        return res.json({isSuccess: false, code: 2002, message: "유효하지 않은 닉네임 형식 입니다."});
    
    if (!regexEmail.test(email)) 
        return res.json({isSuccess: false, code: 2003, message: "이메일 형식을 정확하게 입력해주세요"});
    
    //DB에 존재하는 유저의 이메일인지 검토
    const [userInfoRows] = await userDao.selectUserInfo(email)

    if (userInfoRows.length == 0) 
        return res.json({isSuccess: false, code: 3001, message: "가입되지 않은 이메일 입니다."});
    
    if (userInfoRows[0].nickname != nickname) 
        return res.json(
            {isSuccess: false, code: 3002, message: "올바르지 않은 닉네임 입니다. 가입하실 때 입력한 닉네임을 입력해 주세요."}
        );
...
exports.patchProfile = async function (req, res) {
    ...
    //둘중에 하나라도 입력이 안되면 에러 발생
    if (!image && !nickname) 
        return res.json(
            {isSuccess: false, code: 2000, message: "변경 할 닉네임 또는 이미지 중 하나를 입력해 주세요."}
        );
    ...
    try {
        ...
        return res.json({isSuccess: true, code: 1000, message: "프로필 변경 완료"});
    } catch (err) {
        logger.error(`App - patchImage error\n: ${err.message}`);
        return res
            .status(2010)
